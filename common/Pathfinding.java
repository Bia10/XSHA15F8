...
public class Pathfinding {
	...
	public static int newCaseAfterPush(Carte map, Carte.Case CCase, Carte.Case TCase, int value)
	  {
	...
	public static boolean checkLoS(Carte map, int cell1, int cell2,Fighter fighter, boolean isPeur)
	{
		...
		if(CellsToConsider == null) {
			Ancestra.printDebug("Liste des cases pouvant gener NULL");
			return true;
		}
		//Ancestra.printDebug("Nouvelles cases : ");
		for(Integer cellID : CellsToConsider) {
			...
			if(map.getCase(cellID) != null)
				if(!map.getCase(cellID).blockLoS() || ( !map.getCase(cellID).isWalkable(false) && isPeur )) {
					//Ancestra.printDebug("Il y a une case au moins qui gene la ligne de vue");
					return false;
				}
		}
