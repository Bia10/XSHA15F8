	...
	public void apply(Personnage perso, Personnage target, int itemID, int cellid)
	{
		...
		case -2://créer guilde
			...
			SocketManager.GAME_SEND_Im_PACKET(perso, "022;"+-1+"~"+1575);
				...
				SocketManager.GAME_SEND_MESSAGE(perso, "Pour pouvoir créer une guilde, il faut possèder une Guildalogemme", Ancestra.CONFIG_MOTD_COLOR);
			}
		break;
			...
			case 8://Ajouter une Stat Intelligence | Vitalite
				try
				{
					//perso.get_isTalkingWith() Après jevois pas..
					if(target==null&&itemID==-1&&cellid>-1)//Si la cellid correspond a la reponse(passage d'argument)
					{
						...
						{
							SocketManager.GAME_SEND_MESSAGE(perso,"<b>Erreur</b> - Vous avez déjà été boosté.", Ancestra.COLOR_BLEU2);
							return;
						}
			...
			case 104://Don d'objet (cadeau à la connexion, arg : IDdelitem
				...
				SocketManager.GAME_SEND_MESSAGE(perso, "Vous avez reçu un cadeau sur votre compte !", Ancestra.CONFIG_MOTD_COLOR);
			break;
				...
				try {
					...
					if (alineacion == 0 || alineacion == 3) {
						SocketManager
								.GAME_SEND_MESSAGE(
										perso,
										"Vous n'avez pas l'alignement nécessaire pour poser un prisme.",
					...
					if (subarea.get_alignement() != 0 || !subarea.getConquistable()) {
						SocketManager
								.GAME_SEND_MESSAGE(
										perso,
										"La zone est déjà sous un alignement.",
										Ancestra.CONFIG_MOTD_COLOR);
						return;
					}
			...
			case 170: // Ajouter un titre au joueur 
			    ...
			    {	
					...
					target.set_title(titulo);
			    	    ...
			    	    SocketManager.GAME_SEND_MESSAGE(perso, "<b>Tu possèdes un nouveau titre !</b>", Ancestra.CONFIG_MOTD_COLOR);
						SocketManager.GAME_SEND_STATS_PACKET(perso);
						SQLManager.SAVE_PERSONNAGE(perso, false);
					}catch(Exception e){GameServer.addToLog(e.getMessage());};
				
			    break;
			case 260://Téléportation en parlant a un pnj avec level minimum requis
				try
				{
					short newMapID = Short.parseShort(args.split(",")[0]);
					int newCellID = Integer.parseInt(args.split(",")[1]);
