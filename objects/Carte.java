...
public class Carte {
	...
	private Map<Integer,Case> 		_cases 			= new HashMap<Integer,Case>();
	private Map<Integer,Fight> 		_fights 		= Collections.synchronizedMap(new HashMap<Integer,Fight>());
	private ArrayList<MobGrade> 	_mobPossibles 	= new ArrayList<MobGrade>();
	private Map<Integer,MobGroup> 	_mobGroups 		= Collections.synchronizedMap(new HashMap<Integer,MobGroup>());
	private Map<Integer,MobGroup> 	_fixMobGroups 	= new HashMap<Integer,MobGroup>();
	private Map<Integer,NPC>		_npcs	 		= new HashMap<Integer, NPC>();
	int _nextObjectID = -1;
	private byte _X = 0;
	private byte _Y = 0;
	...
	public static class Case
	{
		...
		public Case(Carte a_map,int id,boolean _walk,boolean LoS, int objID)
		{
			...
			a_map.add_atelier(Constants.JOB_ATELIER(objID));
		}
		...
		{
			if(_persos == null) _persos =Collections.synchronizedMap( new HashMap<Integer, Personnage>());
			_persos.put(perso.get_GUID(),perso);
			
		}
		public void addFighter(Fighter fighter)
		{
			if(_fighters == null) _fighters =Collections.synchronizedMap( new HashMap<Integer, Fighter>());
		...
		public void startAction(Personnage perso, GameAction GA)
		{
			...
			switch(actionID)
			{
				...
				case 170: //Livre de métiers
					String l = "";
					boolean i = false;
					for(int b : perso.get_curCarte().get_atelier()){
						if(i)l+=";";
						l += b;
						i = true;
					}
	...
	public void spawnNewGroup(boolean timer, int cellID, String groupData,
		...
		if (Ancestra.CONFIG_DEBUG)
			GameServer.addToLog("Groupe de monstres ajoutes sur la map: " + _id
					+ " ID: " + _nextObjectID);
	...
	public String getGMsPackets()
	{
		...
		for(Case cell : _cases.values())
		{
			...
			{
				if(perso==null||!perso.isOnline()||perso.get_compte()==null||!perso.get_compte().isOnline())
				{
					dustbin.add(perso.get_GUID());
					continue;
				}
				packet.append("GM|+").append(perso.parseToGM()).append('\u0000');
			}
			for(Integer i:dustbin)
			{
				if(cell.getPersos().containsKey(i))cell.getPersos().remove(i);
			}
			dustbin.clear();
		}
		return packet.toString();
	...
	public String getMobGroupGMsPackets()
	{
		...
		//Ancestra.printDebug(packet.toString());
		return packet.toString();
	}
	...
	public String getNpcsGMsPackets(Personnage p)
	{
	...
	public String getPrismaGMPacket() {
