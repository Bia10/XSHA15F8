...
public class Metier {
	...
	public static class JobAction
	{
		...
		public void craft() {
			...
			//on retire les items mis en ingrï¿½dients
			for (Entry<Integer, Integer> e : _ingredients.entrySet()) {
				...
				//Si la quantitï¿½ est trop faible
				...
				//On calcule la nouvelle quantitï¿½
				...
				//on ajoute le couple tID/qua a la liste des ingrï¿½dients pour la recherche
			...
			if (!success)//Si echec
			{
				...
				//Si signï¿½ on ajoute la ligne de Stat "Fabriquï¿½ par:"
				if (signed) {
				...
				for (Entry<Integer, Objet> entry : _P.getItems().entrySet()) {
					...
					if (obj.getTemplate().getID() == newObj.getTemplate().getID()
							...
							&& obj.getPosition() == Constants.ITEM_POS_NO_EQUIPED)//Si meme Template et Memes Stats et Objet non ï¿½quipï¿½
					{
		...
		private void doFmCraft() {
			...
			for (int guid : _ingredients.keySet()) {
				...
				default:// Si pas runes ou popo, et qu'il a un cout en PA, alors
						// c'est une arme (une vérification du type serait
						// préférable)
		...
		/*	if (isElementChanging > 0 && isStatsChanging == 0)// Si changement
																		...
																		// stats
			{
				...
				SocketManager.GAME_SEND_MESSAGE(_P, "Vous pouvez toujours ajouter "
						...
						+ " stats avec une certitude de réussir.", "009900");

				// 2 cas : Réussite Totale
				// ou échec total : la FM n'a pas réussi et les bonus de toutes
				// les caractéristiques diminuent proportionnellement à la
				// puissance à la puissance de la rune. Utiliser de grosses
				// runes est donc risqué.
			...
			if (!success)// Si echec
			{
																	...
																	// l'élément
				{
					for (SpellEffect SE : obj.getEffects()) {
						...
						try {
							...
							SE.setArgs(newArgs);// on modifie les propriétés du
												// SpellEffect
							SE.setEffectID(stat);// On change l'élement
													// d'attaque
																			...
																			// (rune)
				{
					...
					if (ViewActualStatsItem(obj, stats) == 2)// Le stats existe
																...
																// en négatif
					{
						// Réduit les stats négatifs si réussit jusqu'a leur
						// disparitions
				...
				SocketManager.GAME_SEND_IO_PACKET_TO_MAP(_P.get_curCarte(),
						_P.get_GUID(), "+" + tID);// Icone de réussite
