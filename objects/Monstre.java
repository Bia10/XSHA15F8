...
public class Monstre
{
	...
	public static class MobGroup
	{
		...
		public String parseGM()
		{
			...
			if (Ancestra.CONFIG_ACTIV_ETOILE == true)
			{
			toreturn.append(get_bonusValue());// bonus en pourcentage (étoile/20%) // Actuellement 1%/min
			}
		...
		public int get_bonusValue() {
			int toReturn = (int)get_timeElapsedInSeconds()/Ancestra.CONFIG_SECONDS_FOR_BONUS;
			if(toReturn > Ancestra.CONFIG_BONUS_MAX)
				toReturn = Ancestra.CONFIG_BONUS_MAX;
			return (int)get_timeElapsedInSeconds()/Ancestra.CONFIG_SECONDS_FOR_BONUS;
		}	
	...
	public static class MobGrade
	{
		...
		public MobGrade(Monstre aTemp, int Agrade, int Alevel,int aPA,int aPM, String Aresist, String Astats, String Aspells,int pdvMax,int aInit, long xp)
		{
			template = aTemp;
			grade = Agrade;
			level = Alevel;
		...
		private MobGrade(Monstre template2, int grade2, int level2, int pdv2,int pdvmax2,int aPA,int aPM, Map<Integer, Integer> stats2,Map<Integer, SortStats> spells2,long xp)
		{
			template = template2;
			grade = grade2;
			level = level2;
		...
		for(int n = 0; n<11; n++)
		{
			try
			{
				...
				try
				{
					...
					try
					{
						xp = Long.parseLong(xpstr.split("\\|")[n]);
