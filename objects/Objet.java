...
public class Objet {
	...
	/* *********FM SYSTEM********* */
	public String parseFMStatsString(String statsstr, Objet obj, int add,
			boolean negatif) {
		StringBuilder stats = new StringBuilder();
		boolean isFirst = true;
		for (SpellEffect SE : obj.Effects) {
			...
			String[] infos = SE.getArgs().split(";");
			try {
				stats.append(Integer.toHexString(SE.getEffectID())).append("#")
						.append(infos[0]).append("#").append(infos[1])
						.append("#0#").append(infos[5]);
			} catch (Exception e) {
				e.printStackTrace();
				continue;
			}
		...
		for (Entry<Integer, Integer> entry : obj.Stats.getMap().entrySet()) {
			...
			if (Integer.toHexString(entry.getKey()).compareTo(statsstr) == 0) {
				...
				stats.append(Integer.toHexString(entry.getKey())).append("#")
						...
						.append(add).append("#0#0#").append(jet);
			} else {
				String jet = "0d0+" + entry.getValue();
				stats.append(Integer.toHexString(entry.getKey())).append("#")
						.append(Integer.toHexString(entry.getValue()))
						.append("#0#0#").append(jet);
			}
			isFirst = false;
		}
		...
		for (Entry<Integer, String> entry : obj.txtStats.entrySet()) {
			if (!isFirst)
				stats.append(",");
			stats.append(Integer.toHexString(entry.getKey())).append("#0#0#0#")
	...
	public String parseFMEchecStatsString(Objet obj, double poid) {
		...
		for (Entry<Integer, Integer> entry : obj.Stats.getMap().entrySet()) {
			// En cas d'echec les stats négatives
			// Chance,Agi,Intel,Force,Portee,Vita augmentes
			int newstats = 0;

			if (entry.getKey() == 152 || entry.getKey() == 154
					|| entry.getKey() == 155 || entry.getKey() == 157
				...
				if (newstats > Metier.getBaseMaxJet(obj.getTemplate().getID(),
						Integer.toHexString(entry.getKey()))) {
					newstats = Metier.getBaseMaxJet(obj.getTemplate().getID(),
							Integer.toHexString(entry.getKey()));
				}
			} else {
			...
			if (!isFirst)
				stats.append(",");
			stats.append(Integer.toHexString(entry.getKey())).append("#")
					.append(Integer.toHexString(newstats)).append("#0#0#")
					.append(jet);
			isFirst = false;
		}
	...
	public static int getPoidOfActualItem(String statsTemplate)// Donne le poid
																// de l'item
																// actuel
	{
		int poid = 0;
		...
		for (String s : splitted) {
			...
			int multi = 1;
			if (statID == 118 || statID == 126 || statID == 125
					|| statID == 119 || statID == 123 || statID == 158
					|| statID == 174)// Force,Intel,Vita,Agi,Chance,Pod,Initiative
			{
				multi = 1;
			} else if (statID == 138 || statID == 666 || statID == 226
					|| statID == 220)// Domages %,Domage renvoyé,Piège %
			{
				multi = 2;
			} else if (statID == 124 || statID == 176)// Sagesse,Prospec
			{
				multi = 3;
					...
					|| statID == 243 || statID == 244)// Ré Feu, Air, Eau,
														// Terre, Neutre
			{
				multi = 4;
			} else if (statID == 210 || statID == 211 || statID == 212
					|| statID == 213 || statID == 214)// Ré % Feu, Air, Eau,
														// Terre, Neutre
			{
				multi = 5;
	...
	public static int getPoidOfBaseItem(int i)// Donne le poid de l'item actuel
	{
		...
		ResultSet RS;
		try {
			RS = SQLManager.executeQuery(
					"SELECT statsTemplate from `item_template` WHERE `id`='"
							+ i + "';", Ancestra.STATIC_DB_NAME);
			RS.next();
