...
public class Personnage {
	...
	public long whenKick4nullVote = -1;
	private int _accID;
	...
	private StringBuilder actionsReception;
	public long whenILeaveFaction = -1;
 ...
 public Collection<SortStats> getSorts()
 {
	 ...
	 public StringBuilder getActionsRecepted()
	 {
		 return this.actionsReception;
	 }
	 public void addActionRecepted(int actionID)
	 {
		 if(hasReceivedAction(actionID))return;
		 actionsReception.append("[act:");
		 actionsReception.append(actionID);
		 actionsReception.append("]");
	 }
	 public void addReponseRecepted(int reponseID)
	 {
		 if(hasReceivedReponse(reponseID))return;
		 actionsReception.append("[rep:");
		 actionsReception.append(reponseID);
		 actionsReception.append("]");
	 }
	 public void removeActionRecepted(int actionID)
	 {
		 if(!hasReceivedAction(actionID))return;
		 String strID = "[act:"+actionID+"]";
		 int pos = actionsReception.indexOf(strID);
		 actionsReception.replace(pos, pos+strID.length(), "");
	 }
	 public void removeReponseRecepted(int reponseID)
	 {
		 if(!hasReceivedReponse(reponseID))return;
		 String strID = "[rep:"+reponseID+"]";
		 int pos = actionsReception.indexOf(strID);
		 actionsReception.replace(pos, pos+strID.length(), "");
	 }

	public boolean hasReceivedAction(int actionID)
	{
		return actionsReception.indexOf("[act:"+actionID+"]")!=-1;
		
	}
	public boolean hasReceivedReponse(int reponseID)
	{
		return actionsReception.indexOf("[rep:"+reponseID+"]")!=-1;
	}
	public static class traque 
	{
		private long _time;
		private volatile Personnage _traqued;
	...
	{
		private CopyOnWriteArrayList<Personnage> _persos = new CopyOnWriteArrayList<Personnage>();
		private volatile Personnage _chief;
		...
		public CopyOnWriteArrayList<Personnage> getPersos()
		{
			return _persos;
		}
	...
	public static class Stats
	{
		...
		public Stats(boolean addBases,Personnage perso)
		{
			...
			if(!addBases)return;
			Effects.put(Constants.STATS_ADD_PA,  perso.get_lvl()<100?Ancestra.CONFIG_PA_DEPART:Ancestra.CONFIG_PA_DEPART + 1);
			Effects.put(Constants.STATS_ADD_PM, Ancestra.CONFIG_PM_DEPART);
			Effects.put(Constants.STATS_ADD_PROS, perso.get_classe()==Constants.CLASS_ENUTROF?Ancestra.CONFIG_DEPART_PP_ENU:100);
			Effects.put(Constants.STATS_ADD_PODS, Ancestra.CONFIG_PODS_DEPART);
			Effects.put(Constants.STATS_CREATURE, Ancestra.CONFIG_INVOCATION_DEPART);
			Effects.put(Constants.STATS_ADD_INIT, Ancestra.CONFIG_INITIATIVE_DEPART);
		}
	...
	public Personnage(int _guid, String _name, int _sexe, int _classe,
			...
			int mountXp,int mount,int honor,int deshonor,int alvl,String z, byte title, int wifeGuid, String actionsReception)
	{
		this._GUID = _guid;
		this._name = _name;
		this._sexe = _sexe;
		...
		this.actionsReception = new StringBuilder(actionsReception);
	}
	...
	public static Personnage CREATE_PERSONNAGE(String name, int sexe, int classe, int color1, int color2, int color3,Compte compte)
	{
		...
		if (Ancestra.ACTIVER_CINEMATIQUE)
		{
		SocketManager.GAME_SEND_TB_Packet(perso);
		}
		if (Ancestra.CONFIG_ACTIVER_STATS_2)
		{
			SocketManager.PACKET_POPUP_DEPART(perso, "<b>Bienvenue à vous !</b>"
			+						"\nTutoriel caractéristiques 2.0"
			+						"<img src='http://img11.hostingpics.net/pics/688915Tutoriel2.png' title='Tutoriel'/>"
			+						"															"//Pour éviter les décalages méchants :)
			+						"L'image ci-dessus indique comment monter ses caractéristiques. "
			+						"\nLe code inscrit ajoute 10 points dans le type de caractéristique choisi."
			+						"\nAttention: n'oubliez pas de remplir toutes les cases");
			
			return perso;
		}
		
		if(Ancestra.START_AVEC_PANO_BIENVENUE) {
			Objet obj1 = World.getObjTemplate(Ancestra.CONFIG_ITEM_BIENVENUE).createNewItem(1, false);
			Objet obj2 = World.getObjTemplate(Ancestra.CONFIG_ITEM_BIENVENUE2).createNewItem(1, false);
			Objet obj3 = World.getObjTemplate(Ancestra.CONFIG_ITEM_BIENVENUE3).createNewItem(1, false);
			Objet obj4 = World.getObjTemplate(Ancestra.CONFIG_ITEM_BIENVENUE4).createNewItem(1, false);
			Objet obj5 = World.getObjTemplate(Ancestra.CONFIG_ITEM_BIENVENUE5).createNewItem(1, false);
			Objet obj6 = World.getObjTemplate(Ancestra.CONFIG_ITEM_BIENVENUE6).createNewItem(1, false);
			Objet obj7 = World.getObjTemplate(Ancestra.CONFIG_ITEM_BIENVENUE7).createNewItem(1, false);
			World.addObjet(obj1, true);
			World.addObjet(obj2, true);
			World.addObjet(obj3, true);
			World.addObjet(obj4, true);
			World.addObjet(obj5, true);
	...
	public boolean learnSpell(int spellID,int level,boolean save,boolean send)
	{
		...
		if(send)
		{
			...
			SocketManager.GAME_SEND_Im_PACKET(this, "03;"+spellID);//On send autre chose^^
		}
	...
	public void usarPrisma(String packet) {
		...
		int costo = Formulas.calculZaapCost(_curCarte, World.getCarte(mapaID));
		if (mapaID == _curCarte.get_id())
			costo = 0;
		if (_kamas < costo) {
			SocketManager
					.GAME_SEND_MESSAGE(
							this,
							"No tienes las suficientes kamas para realizar esta acción.",
	...
	public void OnJoinGame()
	{
		...
		if(!Ancestra.CONFIG_MOTD.equals(""))//Si le motd est notifié
		{
			...
			/**SocketManager.PACKET_MESSAGE_BIENVENUE_INFOS(this, "Envie de gagner des points ?"
					+										   "\nVoter pour le serveur et gagnez des items boutiques !" 
					+											 "\n<iframe width='480' height='360' src='http://127.0.0.1/onemu'</iframe> ");
	...
	{
		if(Ancestra.CONFIG_DEBUG)GameServer.addToLog("Perso "+_name+": tentative de boost stat "+stat);
		int value = 0;
		switch(stat)
		{
			case 10://Force
	...
	{
		if(Ancestra.CONFIG_DEBUG)GameServer.addToLog("Perso "+_name+": tentative de boost stat "+stat + " " + countVal + " fois");
	...
	public void levelUp(boolean send,boolean addXp)
	{
		...
		_lvl++;
		_capital+=Ancestra.CONFIG_CARACTERISTIQUES_PAR_LEVEL;
		_spellPts+=Ancestra.CONFIG_POINTS_SORT_PAR_LEVEL;
		_PDVMAX += Ancestra.CONFIG_VITALITE_PAR_LEVEL;
		_PDV = _PDVMAX;
		
		if (Ancestra.CONFIG_ACTIV_GAGNER_PA == true) //Si la variable est activée
		{
		if(_lvl == Ancestra.CONFIG_LEVEL_POUR_GAGNER_PA)
			_baseStats.addOneStat(Constants.STATS_ADD_PA, Ancestra.CONFIG_NOMBRE_PA);
		}
		if (Ancestra.CONFIG_ACTIV_GAGNER_PA2 == true) //Si la variable est activée
		{
		if(_lvl == Ancestra.CONFIG_LEVEL_POUR_GAGNER_PA2)
			_baseStats.addOneStat(Constants.STATS_ADD_PA, Ancestra.CONFIG_NOMBRE_PA2);
		}
		if (Ancestra.CONFIG_ACTIV_GAGNER_PM == true) //Si la variable est activée
		{
		if(_lvl == Ancestra.CONFIG_LEVEL_POUR_GAGNER_PM)
			_baseStats.addOneStat(Constants.STATS_ADD_PM, Ancestra.CONFIG_NOMBRE_PM);
		}
		if (Ancestra.CONFIG_ACTIV_GAGNER_PM2 == true) //Si la variable est activée
		{
		if(_lvl == Ancestra.CONFIG_LEVEL_POUR_GAGNER_PM2)
			_baseStats.addOneStat(Constants.STATS_ADD_PM, Ancestra.CONFIG_NOMBRE_PM2);
		}
		/** News spells for Asterions **/

		if (Ancestra.CONFIG_ACTIVER_NOUVEAUX_SORTS == true) //Si la variable est activée
		{
				
				if(get_lvl() == 1000)
				{
					learnSpell(15000, 1,true,false);//Asteoboost
					SocketManager.GAME_SEND_MESSAGE(this, "Vous avez appris <b>Asteoboost</b>", Ancestra.COLOR_BLEU);
					return;
				}
				if(get_lvl() == 2000)
				{
					learnSpell(15001, 1,true,false);//Asteodown
					SocketManager.GAME_SEND_MESSAGE(this, "Vous avez appris <b>Asteodown</b>", Ancestra.COLOR_BLEU);
					return;
				}
				if(get_lvl() == 3000)
				{
					learnSpell(15002, 1,true,false);//Asteohide
					SocketManager.GAME_SEND_MESSAGE(this, "Vous avez appris <b>Asteohide</b>", Ancestra.COLOR_BLEU);
					return;
				}
				if(get_lvl() == 4000)
				{
					learnSpell(15003, 1,true,false);//Asteoglyphe
					SocketManager.GAME_SEND_MESSAGE(this, "Vous avez appris <b>Asteoglyphe</b>", Ancestra.COLOR_BLEU);
					return;
				}
				if(get_lvl() == 5000)
				{
					learnSpell(15004, 1,true,false);//Asteopoisse
					SocketManager.GAME_SEND_MESSAGE(this, "Vous avez appris <b>Asteopoisse</b>", Ancestra.COLOR_BLEU);
					return;
				}
				if(get_lvl() == 6000)
				{
					learnSpell(15005, 1,true,false);//Asteorion
					SocketManager.GAME_SEND_MESSAGE(this, "Vous avez appris <b>Asteorion</b>", Ancestra.COLOR_BLEU);
					return;
				}
				if(get_lvl() == 7000)
				{
					learnSpell(15006, 1,true,false);//Asteoplak
					SocketManager.GAME_SEND_MESSAGE(this, "Vous avez appris <b>Asteoplak</b>", Ancestra.COLOR_BLEU);
					return;
				}
				if(get_lvl() == 8000)
				{
					learnSpell(15007, 1,true,false);//Asteoria
					SocketManager.GAME_SEND_MESSAGE(this, "Vous avez appris <b>Asteoria</b>", Ancestra.COLOR_BLEU);
					return;
				}
				if(get_lvl() == 9000)
				{
					learnSpell(15008, 1,true,false);//Asteovok
					SocketManager.GAME_SEND_MESSAGE(this, "Vous avez appris <b>Asteoplak</b>", Ancestra.COLOR_BLEU);
					return;
				}
				if(get_lvl() == 10000)
				{
					learnSpell(15009, 1,true,false);//Asterion
					SocketManager.GAME_SEND_MESSAGE(this, "Vous avez appris <b>Asterion</b>", Ancestra.COLOR_BLEU);
					return;
				}
		}
	...
	public String analizarPrismas() {
		String str = "";
		Prisma prisma = World.getPrisma(_curCarte.getSubArea().getPrismaID());
		if (prisma == null)
			str = "-3";
	...
	public void teleport(short newMapID, int newCellID)
	{
		...
		{
			SocketManager.PACKET_POPUP_DEPART(this, "Vous ne pouvez pas effectuer cette action en prison");
			return;
		}
		if(PW != null)
		{
	...
	public void leaveFaction()
	{
		try
		{
			...
			whenILeaveFaction = System.currentTimeMillis();
			String[] infos = _savePos.split(",");
			teleportWithoutBlocked(Short.parseShort(infos[0]), Integer.parseInt(infos[1]));
		}catch(Exception e){};
	}
	...
	public String stringListaPrismas() {//Par exemple la pas de stringbuilder donc + lent mais osef tu utilise peu.
		String map = _curCarte.get_id() + "";
		String str = map + "";
		int SubAreaID = _curCarte.getSubArea().get_area().getSuperArea()
				.get_id();
		for (Prisma prisma : World.TodosPrismas()) {
			if (prisma.getAlineacion() != _align)
				continue;
			short mapaID = prisma.getMapa();
			if (World.getCarte(mapaID) == null)
				continue;
			if (World.getCarte(mapaID).getSubArea().get_area().getSuperArea()
					.get_id() != SubAreaID)
				continue;
			if (prisma.getEstadoPelea() == 0 || prisma.getEstadoPelea() == -2) {
				str += "|" + mapaID + ";*";
			} else {
				int costo = Formulas.calculZaapCost(_curCarte,
						World.getCarte(mapaID));
				if (mapaID == _curCarte.get_id())
					costo = 0;
				str += "|" + mapaID + ";" + costo;
			}
	...
	*/
	...
	public void function_leaveEnnemyFaction()
	{
		if(!Ancestra.ALLOW_TELEPORT_ENEMY_FACTION)return;
		if(!isInEnnemyFaction)return;//pas en prison on fait pas la commande
		long compar = System.currentTimeMillis() - (enteredOnEnnemyFaction + 60*1000*10);
		if(compar >= 0)
		{
			leaveFaction();
			SocketManager.PACKET_POPUP_DEPART(this, "Vous venez d'être libéré de prison après 10 minutes d'attente.");
			//message comme quoi il est sortit
		}else
		{
			long restant = -compar;
			if(restant <= 1000)restant= 1000;
			SocketManager.PACKET_POPUP_DEPART(this, "Vous devez attendre encore "+ restant/1000+" secondes en prison.");
			//message comme quoi il doit attendre
		}
	}
	
	public void function_leaveEnnemyFactionAndPay()
	{
		if(!Ancestra.ALLOW_TELEPORT_ENEMY_FACTION)return;
		if(!isInEnnemyFaction)return;//pas en prison on fait pas la commande
		long compar = System.currentTimeMillis() - (enteredOnEnnemyFaction + 60*1000*10);
		...
		{
			leaveFaction();
			SocketManager.PACKET_POPUP_DEPART(this, "Vous avez atteint le délais d'attente, vous êtes libéré.");
			//message comme quoi il plus besoin de payer et qu'il est sortit
		}else
		{
			if(_honor >= 250)
			{
				int xPerte = _honor/10;
				this.addHonor(-xPerte);
				leaveFaction();
				SocketManager.PACKET_POPUP_DEPART(this, "Vous venez d'échanger "+xPerte+" points d'honneur contre votre libération<br />");
				//message perdu 10% de l'honneur donc xPerte;
			}else
			{
				SocketManager.PACKET_POPUP_DEPART(this, "Vous n'avez pas assez d'honneur pour vous libérer. Vous devez attendre...");
				//message comme quoi contraint d'attendre
			}
		}
	}
	public void teleportFaction(int factionEnnemy)
	{
		short mapID = 0;
		int cellID = 0;
		if(Ancestra.ALLOW_TELEPORT_ENEMY_FACTION)
		{
			...
			{
				case 1 ://bonta
					mapID = Short.parseShort(Ancestra.killedByFaction_BONTA.split(",")[0]);
					cellID = Integer.parseInt(Ancestra.killedByFaction_BONTA.split(",")[1]);
					break;
			
				case 2://brakmar
					mapID = Short.parseShort(Ancestra.killedByFaction_BRAK.split(",")[0]);
					cellID = Integer.parseInt(Ancestra.killedByFaction_BRAK.split(",")[1]);
					break;
			
				case 3://Seriane
					mapID = Short.parseShort(Ancestra.killedByFaction_SERIANE.split(",")[0]);
					cellID = Integer.parseInt(Ancestra.killedByFaction_SERIANE.split(",")[1]);
					break;
			...
			SocketManager.PACKET_POPUP_DEPART(this, "Vous êtes en prison<br />"
					+								"\nVous devez attendre 10 minutes avant de pouvoir sortir de prison" 
					+								"\nFaites .sortir pour sortir dès que possible"
					+								"\nFaites .payer pour donner 10% de votre honneur en échange de votre libération");
		}else
		{
			mapID = (short)8534;
			cellID = 297;
	...
	public void RejoindeWife(Personnage p) 
    {
        if(p == null)
        {
            return;
        ...
        if(dist > 10) 
        {
            SocketManager.GAME_SEND_MESSAGE(this, "Vous etes trop loin de votre amour. Impossible de vous y teleporter.", Ancestra.CONFIG_MOTD_COLOR);
            return;
        } else
        {
	...
	public String parse_tojobbook(int id) {
		...
		Ancestra.printDebug(sm.getTemplate().getId());
		String alpha = "0;0";
