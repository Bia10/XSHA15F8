...
public class SpellEffect
	{
				...
				for(SpellEffect buff : target.getBuffsByEffectID(id))
				{
					switch(id)
					{
						case 9://Derobade
							...
							int newCellID = Pathfinding.newCaseAfterPush(fight, caster.get_fightCell(), target.get_fightCell(), nbrCase);
						...
						case 107://renvoie Dom
							...
							float coef = 2+(target.getTotalStats().getEffect(Constants.STATS_ADD_SAGE)/100);
							int renvoie = 0;
							try
							{
		...
		{
			if(Ancestra.CONFIG_DEBUG)GameServer.addToLog("Effet id: "+effectID+" Args: "+args+" turns: "+turns+" cibles: "+cibles.size()+" chance: "+chance);
			try
			{
			...
			try
			{
				...
				Ancestra.printDebug("-----------------------Erreur challenge (applyToFight())");
