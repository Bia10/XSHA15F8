...
public class RealmThread implements Runnable{
	...
	public RealmThread(String ip, Socket sock)
	{
		try
		{
			this.ip = ip;
	...
	public void kick() {
        try {
            if (RealmServer._ThreadsGroups.containsKey(this.getHostAdress())) {
                RealmServer._ThreadsGroups.get(this.getHostAdress()).delThread(this);
            }
            _in.close();
            _out.close();
            if (_compte != null) {
